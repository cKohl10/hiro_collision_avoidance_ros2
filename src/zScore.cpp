...
// Constructors
zScore::zScore(std::shared_ptr<rclcpp::Node> node_handle, std::string topic_prefix){
    
    topic_prefix = topic_prefix;
    
    //Create all topics we will publish values to
    pub_mean = node_handle->create_publisher<std_msgs::msg::Float64>(topic_prefix + "_zScore_mean", 1);
    pub_positive_threshold = node_handle->create_publisher<std_msgs::msg::Float64>(topic_prefix + "_zScore_positive_threshold", 1);
    pub_negative_threshold = node_handle->create_publisher<std_msgs::msg::Float64>(topic_prefix + "_zScore_negative_threshold", 1);
    pub_signal = node_handle->create_publisher<std_msgs::msg::Float64>(topic_prefix + "_zScore_signal", 1);
    pub_raw_value = node_handle->create_publisher<std_msgs::msg::Float64>(topic_prefix + "_zScore_raw_value", 1);
...
std::tuple<bool, float> zScore::getSignal(double new_value){
  ...
  if(lag_values.size() < lag){
    ...
    lag_values.push_back(new_value);
    // std::cout << "new_val: " <<   new_value  << std::endl;
    return std::make_tuple(signal_found, current_signal);
...
double zScore::getMean(std::vector<double> data){
    ...
    // std::cout << "sum: " << sum << "    Data size: " << data.size() << std::endl; 
    return sum / data.size();
}

std_msgs::msg::Float64 zScore::toROSType(double value) {
